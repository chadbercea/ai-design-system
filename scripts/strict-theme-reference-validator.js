...
const brokenRefs = uniqueRefs.filter(ref => !validKeys.has(ref));

if (brokenRefs.length) {
  console.log('Broken references (not found in core.json):');
  brokenRefs.forEach(ref => console.log(' -', ref));
} else {
  console.log('All references are valid!');
}
console.log(`Checked ${uniqueRefs.length} unique references, ${brokenRefs.length} broken.`); 